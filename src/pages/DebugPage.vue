    ...
    <ion-content>
      <div class="ion-padding">
        <ion-card>
          ...
          <ion-card-content>
            ...
            <div class="actions">
              ...
              <ion-button color="medium" fill="outline" @click="stopWatch" :disabled="!watching"
                >Stop</ion-button
              >
              <ion-button fill="clear" size="small" @click="applyDefaults"
                >Reset Defaults</ion-button
              >
          ...
          <ion-card-content>
            <div class="telemetry">
              ...
              <div class="telemetry-item" v-if="!isWeb">
                ...
                <div class="value">
                  {{ headingMag != null ? `${headingMag.toFixed(0)}°` : '—' }}
                </div>
              </div>
              <div class="telemetry-item" v-if="!isWeb">
                <div class="label">Compass (True)</div>
                <div class="value">
                  {{ headingTrue != null ? `${headingTrue.toFixed(0)}°` : '—' }}
                </div>
              </div>
              <div class="telemetry-item" v-if="!isWeb">
            ...
            <div class="controls">
              ...
              <ion-button color="tertiary" v-if="!locationDebugActive" @click="startAccuracyAudio">
                Start Accuracy Audio
              </ion-button>
              <ion-button color="medium" fill="outline" v-else @click="stopAccuracyAudio">
                Stop Accuracy Audio
              </ion-button>
            </div>
          </ion-card-content>
        ...
        <ion-card>
          ...
          <ion-card-content>
            <ion-button fill="outline" size="small" @click="runDiagnostics"
              >Run Diagnostics</ion-button
            >
            <div class="debug-grid" v-if="diag">
              <div>
                <span class="k">Repos installed:</span>
                <span class="v">{{ diag?.repos ?? '—' }}</span>
              </div>
              <div>
                <span class="k">Query OK:</span> <span class="v">{{ diag?.queryOk ?? '—' }}</span>
              </div>
              <div>
                <span class="k">Create OK:</span> <span class="v">{{ diag?.createOk ?? '—' }}</span>
              </div>
              <div>
                <span class="k">Delete OK:</span> <span class="v">{{ diag?.deleteOk ?? '—' }}</span>
              </div>
              <div v-if="diag?.error">
                <span class="k">Error:</span> <span class="v err">{{ diag?.error }}</span>
              </div>
            </div>
          </ion-card-content>
        </ion-card>
        ...
        <ion-card>
          ...
          <ion-card-content>
            <div class="settle-grid">
              <div>
                <span class="k">Watching:</span> <span class="v">{{ watching }}</span>
              </div>
              <div>
                <span class="k">Target ≤:</span>
                <span class="v"
                  >{{ minAccuracyM }} m ({{ (minAccuracyM * 3.28084).toFixed(0) }} ft)</span
                >
              </div>
              <div>
                <span class="k">Current acc:</span> <span class="v">{{ curAccText }}</span>
              </div>
              <div>
                <span class="k">Best acc:</span> <span class="v">{{ bestAccText }}</span>
              </div>
              <div>
                <span class="k">Fixes seen:</span> <span class="v">{{ fixCount }}</span>
              </div>
              <div>
                <span class="k">Best updates:</span> <span class="v">{{ bestCount }}</span>
              </div>
              <div>
                <span class="k">Countdown:</span> <span class="v">{{ countdownText }}</span>
              </div>
              <div>
                <span class="k">Reason:</span> <span class="v">{{ settleReason }}</span>
              </div>
            </div>
          </ion-card-content>
        </ion-card>
...
const locationDebugActive = ref<boolean>(locationDebugService.isRunning());
...
function startAccuracyAudio() {
  locationDebugService.start();
  locationDebugActive.value = locationDebugService.isRunning();
}

function stopAccuracyAudio() {
  locationDebugService.stop();
  locationDebugActive.value = locationDebugService.isRunning();
}
...
const declinationDeg = computed(() => {
  if (headingMag.value == null || headingTrue.value == null) {
    return null;
  }
...
const declinationText = computed(() =>
  declinationDeg.value != null ? `${declinationDeg.value.toFixed(1)}°` : '—'
);
...
async function restartWatch() {
  ...
  locationStream.configureProvider({
    maximumAgeMs: cfg.options.maximumAge,
    timeoutMs: cfg.options.timeout,
  });
  ...
  if (ok) {
    ...
    watching.value = true;
    if (!sub) {
      sub = locationStream.updates.subscribe((s) => {
        gps.value = {
          lat: s.lat,
          lon: s.lon,
          accuracy: s.accuracy,
...
const formatAccuracyWithProvider = (
  value?: number | null,
  provider?: ProviderKind | null
): string => {
...
const settleReason = computed(() => {
  ...
  if (expired) {
    const isBest =
      gps.value?.ts != null && best.value?.ts != null && gps.value.ts === best.value.ts;
...
async function runDiagnostics() {
  ...
  try {
    ...
    if (!out.repos) {
      throw new Error('Pinia repos not installed');
    }
  ...
  try {
    if (tempId != null) {
      await wps.$repos.waypoints.remove(tempId);
    }
    out.deleteOk = true;
  } catch (e: any) {
...
onMounted(async () => {
  ...
  try {
    ...
    console.warn('[DebugPage] ensurePermissions/restartWatch failed', e);
  }
  if (!isWeb) {
    try {
...
onBeforeUnmount(() => {
  try {
    ...
    console.warn('[DebugPage] removeHeadingListener failed', e);
  }
