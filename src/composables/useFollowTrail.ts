...
export function useFollowTrail(
  waypoints: Ref<TrailPoint[]>,
  position: Ref<PositionLike | null>,
  ...
  function start(fromIndex = 0) {
    ...
    active.value = true;
    announcement.value = waypoints.value.length
      ? `Following trail. Next waypoint: ${waypoints.value[currentIndex.value].name}`
      : '';
  }
  ...
  watch([position, waypoints, active], () => {
    if (!active.value || !position.value || waypoints.value.length === 0) {
      return;
    }
    ...
    const d = haversineDistanceMeters(
      toSafeLatLng(position.value),
      toLatLng(target.lat, target.lon)
    );
    if (d <= thresholdM) {
