<template>
  ...
  <div class="banner-container top" v-if="bannerTop.length">
    <div v-for="a in bannerTop" :key="a.id" class="banner" :class="toColor(a.kind)">
      <span>{{ a.message }}</span>
      <span class="spacer" />
      <ion-button size="small" fill="clear" @click="onUndo(a)" v-if="a.canUndo">{{ a.undoLabel }}</ion-button>
...
const toastActions = computed(() => all.value.filter(a => a.placement === 'top' || a.placement === 'bottom' || a.placement === 'center'));
...
function toToastPosition(p: string) { return p === 'center' ? 'middle' : (p === 'top' ? 'top' : 'bottom'); }
...
function toastButtons(a: ActionItem) {
  const buttons: any[] = [];
  if (a.canUndo) buttons.push({ text: a.undoLabel, role: 'destructive', handler: () => onUndo(a) });
...
.banner-container.top { top: 0; }
.banner-container.bottom { bottom: 0; }
.banner {
  display: flex;
  align-items: center;
...
.banner.success { background: var(--ion-color-success-tint); color: var(--ion-color-success-contrast); }
.banner.warning { background: var(--ion-color-warning-tint); color: var(--ion-color-warning-contrast); }
.banner.danger { background: var(--ion-color-danger-tint); color: var(--ion-color-danger-contrast); }
.spacer { flex: 1; }
</style>
