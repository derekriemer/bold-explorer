...
export class FakeableProvider implements LocationProvider {
  ...
  emitCoords(
    ...
    timePassedMs: number,
    extras?: Partial<Omit<LocationSample, 'lat' | 'lon' | 'timestamp' | 'provider'>>
  ) {
  ...
  walkDirection(
    ...
    if (!this.cur) {
      this.onError?.(
        new Error('FakeableProvider.walkDirection called before an initial position was set')
      );
      return;
    }
    ...
    const dLonDeg =
      metersPerDegLon > 1e-6
        ? (distanceM * Math.sin((bearingDeg * Math.PI) / 180)) / metersPerDegLon
        : 0;
