...
## Phase 1: Pure Logic
- [ ] `tests/unit/utils/geo.spec.ts`
  - [ ] haversineDistanceMeters: identical points = 0 m
  - [ ] haversineDistanceMeters: ~111,320 m per 1° latitude (tolerance)
  - [ ] haversineDistanceMeters: symmetry A→B = B→A
  - [ ] initialBearingDeg: cardinal/quadrant cases, wrap to [0,360)
  - [ ] deltaHeadingDeg: wrap-around and sign correctness (e.g., 350→10 = -20; 10→350 = 20)
- [ ] `tests/unit/services/actions.service.spec.ts`
  - [ ] show: defaults, durable vs timed (durationMs null)
  - [ ] dismiss: removes and calls onDismiss
  - [ ] undo: only when allowed; calls onUndo; removes from undoStack
  - [ ] undoLast: LIFO; clears both stacks appropriately
  - [ ] clearAll: empties actions and undoStack
