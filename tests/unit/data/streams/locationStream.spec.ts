...
import {
  ...
  GeolocationProvider,
  FakeableProvider,
} from '@/data/streams/location/providers';
...
describe('LocationStream helpers', () => {
  it('delegates ensureProviderPermissions when available', async () => {
    ...
    const activeProviderSpy = vi
      .spyOn(providerRegistry, 'getActiveProvider')
      .mockReturnValue(provider);
    const activeGetter = vi
      .spyOn(providerRegistry, 'active$', 'get')
      .mockReturnValue({ subscribe: () => ({ unsubscribe() {} }) } as any);
...
describe('LocationStream settling behaviour', () => {
  ...
  it('emits best-so-far once the settle window elapses', async () => {
    ...
    const sub = stream.updates.subscribe((sample) => {
