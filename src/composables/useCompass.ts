 ...
 */
export function useCompass(opts?: {
  throttleMs?: number;
  ...
  const modeLabel = computed(() => `Compass: ${mode.value === 'true' ? 'TRUE' : 'MAGNETIC'} North`);
  ...
  async function start(): Promise<void> {
    ...
    const source =
      throttleMs > 0 ? compassStream.updates.pipe(throttleTime(throttleMs)) : compassStream.updates;
    sub = source.subscribe((r: HeadingReading) => {
      reading.value = r;
    });
  }
  ...
  async function setLocation(coords: { lat: number; lon: number; alt?: number }): Promise<void> {
    try {
      await compassStream.setLocation(coords);
    } catch (e) {
      console.warn('[useCompass] setLocation failed', e);
    }
  }
