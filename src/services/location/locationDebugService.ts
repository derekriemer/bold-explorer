...
class LocationDebugService {
  ...
  private computeFrequency(accuracy?: number): number {
    ...
    const raw =
      typeof accuracy === 'number' && !Number.isNaN(accuracy) ? accuracy : MAX_ACCURACY_METERS;
    const clamped = Math.min(Math.max(raw, 0), MAX_ACCURACY_METERS);
    const steps = Math.min(MAX_STEPS, Math.round((clamped / MAX_ACCURACY_METERS) * MAX_STEPS));
