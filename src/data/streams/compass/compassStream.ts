...
class CompassStream {
  ...
  async start(opts?: { useTrueNorth?: boolean }): Promise<void> {
    ...
    await provider.start(
      ...
      (e: unknown) => {
        /* surface via logs later */ console.warn('[compass] provider error', e);
      }
    );
    this.active = true;
    ...
    compassProviderRegistry.active$.subscribe(
      async ({ provider: p }: { provider: CompassProvider }) => {
        if (!this.active) {
          return;
        }
        try {
          ...
          console.warn('[compass] swap failed', e);
        }
      }
    );
  }
  ...
  async stop(): Promise<void> {
    ...
    try {
      await compassProviderRegistry.getActiveProvider().stop();
    } catch {}
    this.active = false;
  }
  ...
  async setLocation(coords: { lat: number; lon: number; alt?: number }): Promise<void> {
    try {
      await compassProviderRegistry.getActiveProvider().setLocation?.(coords);
    } catch {}
  }
  ...
  async setTrueNorth(useTrue: boolean): Promise<void> {
    ...
    try {
      await compassProviderRegistry.getActiveProvider().setTrueNorth(this.lastUseTrue);
    } catch {}
  }
}
