...
{
  console.info('[BackgroundGeolocation] ensureAndroidNotificationPermission start', { platform: Capacitor.getPlatform() });
  if (Capacitor.getPlatform() !== 'android') return true;
  if (!Capacitor.isPluginAvailable('LocalNotifications') || typeof LocalNotifications?.checkPermissions !== 'function')
  {
    console.warn('[BackgroundGeolocation] LocalNotifications plugin unavailable; skipping notification permission request');
    return true;
  }
  try
  {
...
export class BackgroundGeolocationProvider implements LocationProvider
{
    ...
    return new Promise((resolve) =>
    {
      ...
      const cleanup = async () =>
      {
        ...
        if (watcherId)
        {
          ...
          catch (err) { console.warn('[BackgroundGeolocation] removeWatcher (snapshot) failed', err); }
        }
        watcherId = null;
      };
  ...
  {
    console.info('[BackgroundGeolocation] start invoked', { alreadyWatching: this.watchId != null });
    ...
    if (!ok)
    {
      onError(createError('Background tracking requires additional permissions.', 'PERMISSION_DENIED'));
      return;
    }
    ...
    } catch (err)
    {
      ...
      const code = (err as any)?.code ?? (typeof (err as any)?.message === 'string' && (err as any).message.includes('implemented') ? 'UNAVAILABLE' : undefined);
      if (code === 'UNAVAILABLE')
      {
