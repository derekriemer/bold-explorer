...
class ProviderRegistry {
  ...
  constructor() {
    ...
    this.state$ = new BehaviorSubject<ProviderState>({
      kind: initialKind,
  ...
  getActiveKind(): ProviderKind {
    return this.state$.getValue().kind;
  }

  /** Get the current active provider instance. */
  getActiveProvider(): LocationProvider {
    return this.state$.getValue().provider;
  }

  /** Return current state (kind + provider). */
  getActive(): ProviderState {
  ...
  private instantiate(kind: ProviderKind): LocationProvider {
    if (this.instances[kind]) {
      return this.instances[kind]!;
    }
    switch (kind) {
      case 'geolocation':
        this.instances[kind] = new GeolocationProvider();
        break;
      case 'mock':
        this.instances[kind] = new FakeableProvider();
        break;
      case 'replay':
        this.instances[kind] = new ReplayProvider(this.replayPoints);
        break;
      case 'background':
        this.instances[kind] = new BackgroundGeolocationProvider();
        break;
    }
    return this.instances[kind]!;
  }

  private resolvePreferredKind(
    ...
    if (
      ...
      !BackgroundGeolocationProvider.isSupported() &&
      !opts.explicit
    ) {
      console.info(
        '[providerRegistry] background provider unsupported; keeping geolocation active'
      );
      return 'geolocation';
    }
