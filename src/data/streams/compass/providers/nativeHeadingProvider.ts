...
export class NativeHeadingProvider implements CompassProvider
{
  ...
  async start (onReading: (r: HeadingReading) => void, onError: (e: unknown) => void): Promise<void>
  {
    ...
    const sub = await Heading.addListener('heading', (r) => { this.onReading?.(r); });
    await Heading.start({ useTrueNorth: this.useTrue });
    this.remove = () => { sub.remove(); void Heading.stop(); };
    this.active = true;
  }
  ...
  {
    if (!this.active) return;
    try { this.remove?.(); } catch (e) { this.onError?.(e); }
    this.remove = null;
    this.active = false;
  }
  ...
  async setTrueNorth (useTrue: boolean): Promise<void>
  {
    ...
    if (!this.active) return; // apply next start
    try
    {
      await Heading.start({ useTrueNorth: this.useTrue });
    } catch (e)
  ...
  {
    try { await Heading.setLocation?.(coords); } catch (e) { this.onError?.(e); }
  }
}
