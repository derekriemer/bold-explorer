    ...
    <ion-content>
      <ion-list inset>
        <template v-if="collections.list.length > 0">
          <ion-item
            v-for="c in collections.list"
            :key="c.id"
            button
            :detail="true"
            @click="toggleOpen(c.id as number)"
          >
            <ion-label>
              <h2>{{ c.name }}</h2>
        ...
        <ion-card>
          ...
          <ion-card-content>
            <div class="section-actions">
              <ion-button size="small" fill="outline" @click="onExport(c.id as number)"
              ...
              <ion-button
                ...
                fill="solid"
                color="primary"
                @click="openAddWaypoints(c.id as number)"
                >Add Waypoints</ion-button
              >
              <ion-button
                size="small"
                fill="solid"
                color="tertiary"
                @click="openAddTrails(c.id as number)"
                >Add Trails</ion-button
              >
            </div>
            <h3>Waypoints</h3>
            <ion-list>
              <ion-item
                v-for="w in collections.contents[c.id as number]?.waypoints ?? []"
                :key="w.id"
              >
                <ion-label>
                  <div class="row">
      ...
      <ion-alert
        v-if="collectionAlertView"
        :is-open="collectionAlertOpen"
        :header="collectionAlertView.header"
        :message="collectionAlertView.message"
        :sub-header="collectionAlertView.subHeader"
        :inputs="collectionAlertView.inputs"
        :buttons="collectionAlertView.buttons"
        :css-class="collectionAlertView.cssClass"
        :backdrop-dismiss="collectionAlertView.backdropDismiss"
        :translucent="collectionAlertView.translucent"
        :animated="collectionAlertView.animated"
        :id="collectionAlertView.id"
...
function buildAddCollectionInputs(): AlertInput[] {
  return [
    {
      name: 'name',
      type: 'text',
...
function onAdd() {
  void collectionAlerts.open('addCollection');
}
  ...
  return {
    ...
    async getItems(): Promise<MultiSelectItem[]> {
      ...
      const existing = new Set(
        (collections.contents[collectionId]?.waypoints ?? []).map((w) => Number(w.id))
      );
...
function makeTrailConfig(collectionId: number): MultiSelectConfig {
  return {
    ...
    async getItems(): Promise<MultiSelectItem[]> {
      const existing = new Set(
        (collections.contents[collectionId]?.trails ?? []).map((t) => Number(t.id))
      );
...
async function openAddWaypoints(collectionId: number) {
  ...
  if (!collections.contents[collectionId]) {
    await collections.loadContents(collectionId);
  }
